    ...
    <div class="fillcontain">
        <div>
            <el-form :inline="true" ref="search_data" :model='search_data'>
                ...
                <el-form-item>
                    <el-button type="primary" size="small" icon="search" @click='changeTime()'>筛选</el-button>
                </el-form-item>
                <el-form-item class="btnRight">
                    <el-button type="primary" size="small" icon="view" v-if="$store.state.user.identity == 1" @click='onAddMoney()'>添加</el-button>
                </el-form-item>
            </el-form>
        </div>
            ...
            <el-table v-if="tableData.length > 0" :data='tableData' max-height="450" border :default-sort="{prop: 'date', order: 'descending'}" style="width: 100%">
                ...
                <el-table-column prop="date" label="创建时间" align='center' width="250" sortable>
                    <template slot-scope="scope">
                        ...
                        <span style="margin-left: 10px">{{ scope.row.date | filterTime }}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column v-if="$store.state.user.identity == 1" prop="operation" align='center' label="操作" fixed="right" width="180">
                    <template slot-scope='scope'>
            ...
            <el-row>
                <el-col :span="24">
                    <div class="pagination">
                        <el-pagination v-if='paginations.total > 0' :page-sizes="paginations.page_sizes" :page-size="paginations.page_size" :layout="paginations.layout" :total="paginations.total" :current-page.sync='paginations.page_index' @current-change='handleCurrentChange' @size-change='handleSizeChange'>
                        </el-pagination>
                    </div>
                </el-col>
            </el-row>
...
export default {
    ...
    filters: {
        filterTime(e) {
            e = new Date(+e)
            return e.getFullYear() + "年" + (e.getMonth() + 1) + "月" + e.getDate() + "日 " + e.getHours() + "点" + e.getMinutes() + "分" + e.getSeconds() + "秒";
        }
    },
    methods: {
        getCorrectTime(e) {
        ...
        onDeleteMoney(row, index) {
            // 删除
            this.$axios.post(`/profile/delete/`, {
                id: row._id
            }).then(res => {
                this.getProfile();
            });
